...
const auxiliaryPanels = Array.from(
    document.querySelectorAll<HTMLElement>('[data-build-calculation-panel], [data-build-tips-panel]'),
);
...
function selectBuild(targetId: string | null, updateUrl = true) {
    ...
    const tipsSection = document.querySelector<HTMLElement>('[data-build-tips-section]');
    const hasTips = auxiliaryPanels.some((panel) => panel.hasAttribute('data-build-tips-panel') && !panel.hidden);
    if (tipsSection) {
        tipsSection.hidden = !hasTips;
    }
